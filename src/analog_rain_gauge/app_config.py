...
class AnalogRainGaugeConfig(config.Schema):
    input_pin = config.Integer(
        "Input Pin", description="Input pin connected to the rain gauge.", minimum=0
    )
    output_pin = config.Integer(
        "Output Pin",
        description="(Optional) Output DO output pin connected to the rain gauge.",
        minimum=0,
        default=None,
    )
    mm_per_pulse = config.Number(
